...
WINDOW *OutputWindow,*InputWindow,*TitleWindow;
char *t1,*t2,*t3,*t4;
...
void init_gui(void){
  ...
  noecho();
	...
	t1 = getAsciiText("resources/title.txt");
	t2 = getAsciiText("resources/from.txt");
	t3 = getAsciiText("resources/from2.txt");
	t4 = (char *)malloc(4096);
	t4 = getAsciiText("resources/pic.txt");
	...
	//create windows 
  OutputWindow = newwin(getmaxy(stdscr)-3, getmaxx(stdscr)/2, 0, getmaxx(stdscr) /2);
  TitleWindow = newwin(getmaxy(stdscr)-3, getmaxx(stdscr)/2, 0, 0);
...
void printInputW(char *message){
    ...
    mvwprintw(InputWindow, getmaxy(InputWindow)-2, 1, message);
...
void drawWindows(void){
  clear();
	...
	printAsciiText(TitleWindow, t1, 1, 1,GREEN_PAIR);
	printAsciiText(TitleWindow, t2, 8, 1, GREEN_PAIR);
	//printAsciiText(TitleWindow, t3, 16, 1, GREEN_PAIR);
	printAsciiText(TitleWindow, t4, 26, 10, RED_PAIR);
	mvwprintw(TitleWindow, 24,2,"type /exit to exit the program");
	printOutputW("","\n");
